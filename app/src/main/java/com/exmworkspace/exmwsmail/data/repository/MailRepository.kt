...
import com.exmworkspace.exmwsmail.data.mail.FolderSyncBatch
import com.exmworkspace.exmwsmail.data.mail.ImapGateway
import com.exmworkspace.exmwsmail.data.mail.OutgoingMessage
import com.exmworkspace.exmwsmail.data.mail.RemoteMessageHeader
...
class MailRepository(
    ...
    private suspend fun applyBatch(folder: FolderEntity, batch: FolderSyncBatch) {
        ...
        if (batch.headers.isEmpty() || batch.syncedUidRange == null) {
            AppLog.d("MailRepo", "applyBatch: empty headers for ${folder.fullName}")
            return
        }
